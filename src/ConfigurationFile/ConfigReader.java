...
public class ConfigReader {
    public HashMap leerArchivo() throws  MissingContentError {
        ...
        while (iterator.hasNext()){
            ...
            if(jsonObject.get("Description")==null) throw new MissingContentError("Description not found in object number " + i);
            ...
            if (!name.matches(a) && !name.matches(b) && !name.matches(n)) throw new MissingContentError("Empty name in object number " + i);
            String description = (String) jsonObject.get("Description");
            if (!description.matches(a) && !description.matches(b) && !description.matches(n)) throw new MissingContentError("Empty description in object number " + i);
            ...
            for (int j =0; j<exits.size();j++){
                if (!exits.get(j).toString().matches(a) && !exits.get(j).toString().matches(b) && !exits.get(j).toString().matches(n)) throw new MissingContentError("Empty exit in object number " + i + ", exit " +(j+1));
            }
